...
#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_MINAH
static
#endif
struct cpufreq_governor cpufreq_gov_minah = {
       .name                   = "minah",
...
static void minah_powersave_bias_init_cpu(int cpu)
{
...
static void minah_powersave_bias_init(void)
{
	int i;
	for_each_online_cpu(i) {
		minah_powersave_bias_init_cpu(i);
	}
}
...
/* cpufreq_minah Governor Tunables */
				   ...
				   unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		this_dbs_info->rate_mult = 1;
		minah_powersave_bias_init_cpu(cpu);
		/*
...
static struct notifier_block minah_qos_dvfs_lat_nb = {
...
static int __init cpufreq_gov_dbs_init(void)
{
	...
	err = pm_qos_add_notifier(PM_QOS_DVFS_RESPONSE_LATENCY,
			    &minah_qos_dvfs_lat_nb);
	if (err)
		return err;

	err = cpufreq_register_governor(&cpufreq_gov_minah);
	if (err) {
...
static void __exit cpufreq_gov_dbs_exit(void)
{
	...
	cpufreq_unregister_governor(&cpufreq_gov_minah);
}
...
MODULE_DESCRIPTION("'cpufreq_minah' - BASED ON ONDEMAND");
MODULE_LICENSE("GPL");

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_MINAH
